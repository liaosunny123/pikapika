...
def download_target_files(access_key_id: str, access_key_secret: str, end_point: str, bucket_name: str,
                          file_path: list[str]) -> list[str]:
...
def upload_target_files(access_key_id: str, access_key_secret: str, end_point: str, bucket_name: str,
                        file_path: list[str], upload_prefix: str) -> list[str]:
    auth = oss2.ProviderAuth(StaticCredentialsProvider(access_key_id, access_key_secret))
    bucket = oss2.Bucket(auth.credentials_provider, end_point, bucket_name)
    ret = []
    for file in file_path:
